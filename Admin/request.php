...
<div class="col-sm-4 mb-5">
  ...
  <?php 
 $sql = "SELECT request_id, request_info, request_des, request_date FROM submitreq_db";
 ...
 if($result->num_rows > 0){
  while($row = $result->fetch_assoc()){
   ...
   echo '<p class="card-text">' . $row['request_des'] . '</p>';
...
if(isset($_REQUEST['close'])){
  $sql = "DELETE FROM submitreq_db WHERE request_id = {$_REQUEST['id']}";
